...
function [ind, x, d] = uniphase_simplex (A, b, c, m, n)  
  ...
  printPhase(phase);
  [ind, u] = simplex(newA, b, newc, m, n + m, newx, phase);
  ...
  phase = 2;
  printPhase(phase);
  [ind, u] = simplex(A, b, c, m, n, x, phase);
...
#=======================================================================
function [ind, v, A, m, x] = simplex(A, b, c, m, n, x, phase)
   ind = 2;   #dummy
   count = 0;
  ...
  printSolucao(x, v, basic, c, ind, m, n, phase);
endfunction
...
function [ind, x] = artificialOut(A, B, InvB, b, basic, Nbasic, x, m, n)
  ...
  while (artificial != 0) 
    ...
    index = 0;  
    j = 1;
    while (index == 0 && j <= n)  #Verifica se a linha da variavel artificial basica xi
...
#=======================================================================
# Esta funcao devolve em m(linha) e n(coluna), a dimensao de A
#=======================================================================
function [m, n] = size(A)
  m = rows(A);
  n = columns(A);
...
function printSolucao(x, v, basic, c, ind, m, n, phase)
  if (phase == 1 || (phase == 2 && ind == 0))
    printf("\nSolucao otima com custo %f:\n", c' * x);
    for (i = 1 : n)
       printf("%d  %f\n",i ,x(i));
    end

  elseif (phase == 2 && ind != 0)
